...
fetch(CMS_ENDPOINT, {
    ...
    .then((data2) => {
      ...
      for (let i = 0; i < cms_desc.length; i++) {
        ...
        // for (let j = 0; j < cms_desc.length; j++) {
      ...
      selected = cms_desc[0]["name"];
      selectedid = cms_desc[0]["id"];
...
button.addEventListener("click", function () {
  ...
  let event_id = selectedid


  fetch (link +"event/"+event_id).
