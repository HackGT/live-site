    ...
    const getEvents = async () => {
      ...
      for (let i = 0; i < allUpcomingEvents.length; i++) {
        if (allUpcomingEvents[i].url !== null) {
          setMainStageEvent(
            new EventInformation(
              allUpcomingEvents[i].id,
              allUpcomingEvents[i].url,
              allUpcomingEvents[i].name,
              allUpcomingEvents[i].tags,
              allUpcomingEvents[i].description
            )
          );
      ...
      setMainStageEvent(
        new EventInformation(
          ...
          [],
          "Check the schedule to see when the next event will go live!"
        )
      );
    };
    getEvents();
