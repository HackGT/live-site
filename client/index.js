define([
  ...
  initializeBJN = function () {
    ...
    BJN.RTCRoster.getPartyCount();
    RemoteVideoOrMask();

    // Get list of A/V on this PC
    BJN.RTCManager.getLocalDevices().then(
      function (devices) {
        BJN.localDevices = devices.available;
        var avail = devices.available;
        console.log(
          "Got local devices, available:" +
            JSON.stringify(avail).substr(0, 35) +
            "..."
        );
  ...
  function PartyVideoMuted(p) {
    return p.attributes.isSendingVideo ? "<img src='media/vcam.png'/>" : " ";
  }

  function PartyAudioMuted(p) {
    return p.attributes.isSendingAudio ? "<img src='media/mic.png'/>" : " ";
  }
  ...
  function cbPartyJoined(p) {
    var r = '<tr id="' + p.cid + '">';
    r += "<td class='rostern'>" + p.attributes.name + "</td>";
    r += "<td class='rosterv'>" + PartyVideoMuted(p) + "</td>";
    r += "<td class='rostera'>" + PartyAudioMuted(p) + "</td>";
    r += "</tr>";
