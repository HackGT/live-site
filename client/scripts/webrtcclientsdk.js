...
*/
  ...
  var joinMeeting = function (meetingParams) {
    if (
      meetingParams.numericMeetingId != "" &&
  ...
  var onLocalConnectionStateChange = function (state) {
    ...
    if (n != participants.length) {
      console.log("Party Count: " + BJN.RTCRoster.getPartyCount());
      participants = [];
      for (let i = 0; i < n; i++) {
        participants.push(BJN.RTCRoster.getParticipant(i).id);
      }
    ...
    /*
      const data = { array: participants };
      fetch("localhost:3000/clicked", {
        method: "POST",
        headers: {
